...
const Settings: React.FC = () => {
  ...
  const SettingRow = ({ label, icon: Icon, enabled = true, onToggle }: { label: string, icon: any, enabled?: boolean, onToggle?: () => void }) => (
    <div className="w-full bg-gray-200 border-4 border-black p-4 mb-4 flex items-center justify-between clip-jagged shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transform hover:-translate-y-1 transition-transform">
        ...
        <button 
          onClick={onToggle}
          className={`w-16 h-8 ${enabled ? 'bg-green-500' : 'bg-red-500'} border-2 border-black flex items-center px-1 cursor-pointer active:scale-95`}
        >
  ...
  return (
    <ScreenContainer className="bg-celo-yellow/80">
      ...
      <div className="p-6 flex flex-col gap-2 max-w-md mx-auto mt-8">
         <SettingRow label="Sound FX" icon={Volume2} enabled={soundFX} onToggle={() => setSoundFX(!soundFX)} />
         <SettingRow label="Music" icon={Music} enabled={music} onToggle={() => setMusic(!music)} />
         <SettingRow label="Haptics" icon={Vibrate} enabled={haptics} onToggle={() => setHaptics(!haptics)} />
